...
class DatingMyVideosViewController: ATCGenericCollectionViewController {
    ...
    init(user: ATCUser,
        ...
        var items: [ATCImage] = []
       // if let igPhotos = user.videos {
     //       items = igPhotos.map({ATCImage($0)})
//}
        let actionImage = ATCImage(localImage: UIImage.localImage("video-call-icon", template: true))
    ...
    private func addImageAlertController() -> UIAlertController {
        let alert = UIAlertController(title: "Add Video".localizedInApp, message: "", preferredStyle: UIAlertController.Style.actionSheet)
        ...
        alert.addAction(UIAlertAction(title: "Take Video".localizedInApp, style: .default, handler: {[weak self] (action) in
            guard let strongSelf = self else { return }
            strongSelf.didTapAddImageButton(sourceType: .camera)
        }))
  
        alert.addAction(UIAlertAction(title: "View Video".localizedInApp, style: .default, handler: {[weak self] (action) in
            guard let strongSelf = self else { return }
            strongSelf.didTapAddImageButton(sourceType: .camera)
            guard let video = strongSelf.user.videos?.first else {
    ...
    fileprivate func didAddVideo(_ url: URL) {
        let hud = CPKProgressHUD.progressHUD(style: .loading(text: "Sending".localizedInApp))
        hud.show(in: view)
        profileUpdater.uploadVideo(videoURL: url, user: user, isProfileVideo: true) { success in
            hud.dismiss()
        }
    }
...
extension DatingMyVideosViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
                ...
                if let mediaType = info[.mediaType] as? String {
                    if mediaType == kUTTypeMovie as String {
                        if let videoURL = info[.mediaURL] as? URL {
                            let url = createTemporaryURLforVideoFile(url: videoURL as NSURL)
                            self.didAddVideo(url as URL)
