//
//  ATCLineChartLineProgress.swift
//  FitnessApp
//
...
public struct ATCLineChartLineProgress: View {
    ...
    var gradient = Gradient(colors: [.purple, .blue])
    var index:Int = 0
    ...
    var stepHeight: CGFloat {
        ...
        if let min = min, let max = max, min != max {
            return (frame.size.height-padding) / CGFloat(min <= 0 ? (max - min) : (max + min))
        }
        return 0
    }
    var path: Path {
    ...
    public var body: some View {
        ZStack {
            if self.showFull && self.showBackground {
                self.closedPath
                    .fill(LinearGradient(gradient: Gradient(colors: [.blue, .white]), startPoint: .bottom, endPoint: .top))
...
struct Line_Previews: PreviewProvider {
    static var previews: some View {
        GeometryReader{ geometry in
            ATCLineChartLineProgress(data: ATCLineChartDataSource(points: [12, -230, 10, 54]),
